...
template<class T>
void onpCalculator(std::string input) {
    MyStack<T> stack;
    T left;
    T right;
    T converted;
    ...
    do{
        ...
        std::cout << "output " << stack.peek() << std::endl; // print peak of stack
        std::cout << "> ";
        getline(std::cin, input); // waiting for input

    }while (input != "00"); // exit code
}
